...
/**
 * Adds a single random, compatible enchantment to the item being enchanted on
 * {@link EnchantItemEvent}.
 *
 * <p>On a successful chance roll a candidate enchant is chosen from the item's
 * full compatible pool, skipping enchants already present on the item or chosen
 * this roll and those that conflict with them. The bonus level is a random 1..max
 * (bounded by the enchant's own max). Reaching the roll counts as an activation
 * attempt (cost/cooldown consumed) whether or not it succeeds, matching the
 * chance-mechanic contract.
 *
 * <p><b>YAML key:</b> {@code core:bonus_enchant}
 * <br>Params: {@code chance} (0-100, percentage to add a bonus enchant)
 */
public final class BonusEnchantMechanic implements SkillMechanic {
    ...
    public boolean execute(Player player, Map<String, Object> params, Event event) {
        ...
        Map<Enchantment, Integer> chosen = enchantEvent.getEnchantsToAdd();
        Enchantment bonus = pickBonus(enchantEvent.getItem(), chosen);
        if (bonus != null) {
            int maxLevel = Math.max(1, bonus.getMaxLevel());
            int level = 1 + (int) (levelRandom.getAsDouble() * maxLevel);
            chosen.put(bonus, Math.min(level, maxLevel));
        }
        return true;
    }

    private Enchantment pickBonus(ItemStack item, Map<Enchantment, Integer> chosen) {
        List<Enchantment> occupied = new ArrayList<>(item.getEnchantments().keySet());
        occupied.addAll(chosen.keySet());
