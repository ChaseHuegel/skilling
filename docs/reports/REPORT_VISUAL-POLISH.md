...
plan catalogs every surface, then proposes concrete, file-anchored improvements — starting with
...
- Lore uses `&7` for neutral, `&a` for values, `&c`/`&8` for costs/requirements, `&e`/`&d`/`&6`
  for per-skill accents — but **not consistently**; e.g. `&b` appears in some success messages,
  `&d`/`&6` vary per skill.
...
- Color handling: keep the current two-`Component` split — filled part colored green
  (`NamedTextColor.GREEN`), empty part gray. Optionally tie the filled color to `display.color`
...
- **Flat overview title:** change `"Skills"` (gold, `SkillMenuBuilder.java:63`) to
  ...
  neutral brand color, but ensure paginated page titles default to `"Skills"` too when
...
- Keep the gray + `· Locked` name, but render `· Locked` in `DARK_GRAY` (already done) and add
  ...
  `unlockLevel` — mirroring the ability-lore lock treatment (`SkillMenuBuilder.buildSkillLore`
