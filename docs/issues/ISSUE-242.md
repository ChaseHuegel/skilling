...
- [x] Make `/skills set` (and the web config editor, if applicable) report that `database.pool_size` requires a restart instead of claiming success — or apply the change live (not feasible for a Hikari pool; prefer the clear restart-required message, mirroring the web `ConfigHandler` `WEB_RESTART_REQUIRED` behavior).
- [x] Add a test asserting the restart-required path surfaces for pool-size changes.
...
- [x] `/skills set database.pool_size N` no longer reports a success that has no effect.
- [x] Live-applicable keys still apply.
