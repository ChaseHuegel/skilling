...
- [ ] Replace every unchecked raw cast in `SkillManager.parseSkill` (and its helpers) with type-checked accessors that throw `IllegalArgumentException` with a descriptive message on non-string/scalar/map/list mismatches.
- [ ] Cover at minimum: `trigger`, `display_name`, `id`, `state`, `items`, `sounds`, and `target`/`tool` filter keys.
- [ ] Keep the existing `instanceof Map` guards and `String.valueOf`-style sites consistent with the new accessors.
- [ ] Add unit tests feeding numeric/scalar values for these keys and assert the failure is `IllegalArgumentException` and that `loadSkills` skips the file with a warning rather than propagating.
...
- [ ] New tests pass: each malformed-value case throws `IllegalArgumentException` from parsing.
- [ ] A folder containing one malformed file plus valid files loads the valid files and logs a warning for the bad one.
- [ ] `./gradlew test` and `./gradlew build` pass.
- [ ] Edge case handled: `LockdownManager` reload completes cleanly when a file in the data folder is malformed.
