...
- [x] `CrowdControlMechanic.execute` (`CrowdControlMechanic.java:38`) passes the raw `radius` parameter into `getNearbyEntities(radius, radius, radius)`, unlike the buff auras which clamp to Bukkit's [0, 32] entity-search cap. Clamp the radius (mirroring `AllyAuraMechanic.clampRadius` / the `clampRadius` added to `AoeEffectMechanic`/`FieldAuraMechanic` in ISSUE-216) before the scan.
- [x] Add a unit test asserting an oversized radius is clamped (e.g. `getNearbyEntities(32, 32, 32)` is used for a radius of 1000) and that the caster is never affected.
...
- [x] `CrowdControlMechanic` radius is bounded to [0, 32].
- [x] New test verifies the clamp and that the caster is excluded.
