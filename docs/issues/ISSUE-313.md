...
- [x] Create `src/main/resources/skills/exploration.yml` (id `exploration`, icon `minecraft:map` or `minecraft:cartography_table`, 6-tier milestone template, polynomial curve) and add it to the `bundledSkills` array and the default `gui.yml`.
- [x] XP sources (do NOT use `chunk_load`; Survival owns it):
...
- [x] Design owner confirms the L100 capstone and the final ability set.
- [x] Bundled-skill auto-sweeps pass; `exploration.yml` parses and is shipped in `bundledSkills` and `gui.yml`.
- [x] Lore placeholder tests pass for `{multiplier}` / `{duration}` / `{radius}`.
- [x] `docs/users/creating-skills.md` shows the exploration sources.
