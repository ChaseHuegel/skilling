...
- [x] A reload that throws (bad skill YAML) leaves resolvers/registries on the previous consistent set.
- [x] Unknown state keys and malformed biome values fail at load, not at runtime.
- [x] `player_placed:true` and `player_placed:false` both behave per their value; stale metadata no longer blocks natural blocks.
- [x] The GUI rejects the same gui.yml layouts the engine rejects, before apply.
