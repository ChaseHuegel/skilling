...
- [x] Fix the `NullPointerException` in `ProjectileReturnMechanic.execute` (`ProjectileReturnMechanic.java:51`) — `player.getEyeLocation()` returns null on the mocked player, so `getDirection()` NPEs. Production players always have an eye location; the fix belongs in the test (stub `getEyeLocation()`), not the mechanic.
- [x] Affected tests:
...
- [x] Optionally assert the drop target location is near the player's eye location in one case, so the mock is meaningful rather than a null-return stub.
...
- **Note:** The mechanic switched to `dropItem` (precise eye-relative placement) in `eb0cbe7`; the tests still verified `dropItemNaturally`, so the verifies were aligned to `dropItem` and `dropItem` is stubbed to return a mock `Item` (the mechanic sets pickup delay/velocity on it).
