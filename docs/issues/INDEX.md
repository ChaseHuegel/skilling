...
- [x] [ISSUE-174](ISSUE-174.md) - Rework `ProjectileReturnMechanic` — preserve projectile metadata, prevent duplication, support arrows
- [x] [ISSUE-121](ISSUE-121.md) - Run damage-cancelling mechanics at an early priority instead of MONITOR
- [x] [ISSUE-123](ISSUE-123.md) - Wire the `level_up` trigger to `SkillingLevelUpEvent` instead of vanilla level changes
- [x] [ISSUE-124](ISSUE-124.md) - Dispatch `BrewEvent` and `PrepareAnvilEvent` so blocked mechanics become reachable
...
- [x] [ISSUE-139](ISSUE-139.md) - Make registries and the skill map thread-safe for reload
- [x] [ISSUE-153](ISSUE-153.md) - Eliminate `ConcurrentModificationException`/torn reads on shared engine state during web reads
...
- [x] [ISSUE-172](ISSUE-172.md) - Follow-up to ISSUE-110 — sort the actual navigation flyout (topbar dropdown) by color then name
