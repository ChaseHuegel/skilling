...
- [x] `ChainBreakMechanic.execute` (`ChainBreakMechanic.java:92-117`) counts only chained blocks in `broken` while the Javadoc (`:24`, shared with `LevelBreakMechanic`) says `chain_limit` is "max total blocks broken including the origin". Make the count match the documented semantics (include the origin in the limit) or change the documentation — pick one contract and test it.
- [x] When `neighbor.breakNaturally(...)` breaks nothing (e.g. an unbreakable same-type block), do not increment `broken` and do not call `ToolDurability.damageOnce` (`:106-111`), so the limit is not consumed and the tool is not worn.
- [x] Add unit tests for both: exact break count at the limit, and zero durability cost when chained blocks fail to break.
...
- [x] `chain_limit: N` yields exactly N total blocks (including origin) broken.
- [x] An unbreakable chained block neither consumes the limit nor the tool durability.
