...
- [x] Stop clearing the whole staging directory on reload success. Capture the applied file list from `applyAndBackup()` and remove only those applied entries (and their staged sources) on success.
- [x] Alternatively, hold a "reload in progress" state that rejects/blocks new stage requests until the rebuild completes.
- [x] Add a test: stage an edit during the apply→clear window and assert it is preserved after the reload completes.
...
- [x] New in-flight-edit preservation test passes.
- [x] `./gradlew test` and `./gradlew build` pass.
- [x] Edge case handled: a normal single-admin flow (no concurrent edit) clears staging as before.
