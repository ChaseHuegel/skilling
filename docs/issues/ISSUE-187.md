...
- [ ] Add `CureVillagerTrigger` (`src/main/java/io/github/chasehuegel/skilling/engine/trigger/impl/`) implementing `SkillTrigger`: key `cure_villager`, event class `EntityTransformEvent`.
- [ ] Register `cure_villager` in `Skilling.registerBuiltinTriggers`.
- [ ] Add a MONITOR handler in `SkillEventListener` (`onCureVillager`) that:
...
- [ ] Javadoc on the trigger class and handler explaining purpose, YAML key, and the offline-player edge.
- [ ] `cure_villager` must not fire for other transform reasons (INFECTION/DROWNED/etc.).
...
- [ ] `./gradlew test` passes, including new tests:
...
- [ ] Edge case handled: conversion player unknown (`getConversionPlayer() == null`) → no dispatch, no throw.
