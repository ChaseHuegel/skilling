...
- [x] Persist the resolved live-file relative path in the deletion marker (or a sidecar mapping) at `stageSkillDeletion` time, and have `applyAndBackup` delete exactly that path.
- [x] Apply the same path resolution to rename-out-of-subfolder flows so the original nested file is removed.
- [x] Add a test: delete `skills/special/blasting.yml` through the API and assert the live file is gone after apply.
...
- [x] New nested-skill delete/rename tests pass.
- [x] `./gradlew test` and `./gradlew build` pass.
- [x] Edge case handled: deleting a flat-path skill behaves exactly as before.
