...
## Context & User Story
- **Goal:** As a server admin, I want to configure every in-game visual element of Skilling (skill tooltip lore, XP bar, ability lines, level-up/unlock messaging, chest titles, guide book, boss bar text, command feedback) from `config.yml` so that my server's branding is consistent and fully under my control.
