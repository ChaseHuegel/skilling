...
- [ ] Add regression tests: a config update round-trip preserves unknown keys; a rate limiter unit test covering a spoofed/forwarded IP (existing `AuthRateLimiterTest` and `ConfigHandlerSecurityTest` to extend).
...
- [ ] A config save round-trip preserves all keys not present in the editor payload.
- [ ] A brute-force burst behind a proxy cannot lock out all clients; per-client IPs still get rate limited.
- [ ] CORS policy does not allow unauthorized cross-origin reads/writes with cached admin credentials.
