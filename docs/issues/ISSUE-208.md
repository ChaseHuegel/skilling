...
- [x] `SkillEventListener.onFish` must only dispatch the `fishing` trigger when `event.getState() == PlayerFishEvent.State.CAUGHT_FISH` (or otherwise gate on the state that represents a completed catch). Casts (`FISHING`), bites (`CAUGHT_ENTITY`), reels, and `FAILED_ATTEMPT` must not grant XP or fire abilities.
- [x] If state gating is the chosen fix, confirm the bundled `fishing.yml` XP source and abilities need no per-source state filter (the single gate covers them all).
- [x] Reconcile `fishing.yml` comment ("Treasure-catch source deferred (no fish-state filter yet)") with the new behavior; the deferred treasure source remains deferred only if the state-gate change does not enable it.
...
- [x] A single cast-and-catch grants exactly one XP reward and fires fishing abilities exactly once; a cast with no catch grants nothing.
